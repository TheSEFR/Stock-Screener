...
def format_market_cap(value: float | None) -> str:
    ...
    if not value:
        return "n/d"
    # Sin decimales desde 1.000 en su unidad ("1.318T" y no "1.318,2T"): con decimal la
    # celda no cabe en una linea y descuadra la tabla (ver SUMMARY_WIDTHS).
    if value >= 1_000_000_000_000:
        scaled = value / 1_000_000_000_000
        return f"{fmt_es(scaled, 0 if scaled >= 1000 else 1)}T"
    if value >= 1_000_000_000:
        scaled = value / 1_000_000_000
        return f"{fmt_es(scaled, 0 if scaled >= 1000 else 1)}B"
...
def risk_label(r):
    failures, missing = core.risk_reasons(r)
    return 'Alto' if failures else 'Rev.' if missing else 'OK'  # 'Revisar' no cabe en la celda y parte la fila
