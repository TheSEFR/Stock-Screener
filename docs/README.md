...
- **Alertas por cambios** (`alerts.py`): compara con la decisión anterior y avisa de nuevas candidatas, salidas, precio que entra en el umbral de estudio, caída del FCL, pérdida de calidad y cambios de prioridad. `--only-changes` evita enviar el mismo informe si no hay cambios, pero **el workflow diario no lo usa: el informe se envía todos los días**. Las alertas se calculan igualmente (`informes/alertas.md`) y el pie del mensaje indica cuántos cambios hay. Se guarda en `informes/alertas.md`.
...
- **Contraste con SEC** (`reconcile.py`): compara ingresos y beneficio neto anuales de Yahoo con `companyfacts` de SEC para emisores en USD. Una discrepancia mayor del 5% baja una «prioridad alta» a `revisar_historial`. Sin `SEC_EDGAR_USER_AGENT` real queda `sin_verificar`. **No probado en vivo**: solo con respuestas simuladas.
...
## Columna Est. y sección 8 (estado de cada acción)

Cada tabla del PDF tiene una columna **Est.** con una letra: **C** candidata, **R** revisar (faltan datos), **D** descartada por riesgo, **N** no cumple, **X** duplicada. Al pulsarla salta a la **sección 8**, que explica en lenguaje llano por qué tiene ese estado y, si tiene valoración, qué dice la sección 6 (`explain.py` traduce los motivos técnicos a frases; un motivo sin traducción se muestra tal cual). La etiqueta `revisar_calidad` de la valoración se llama ahora `revisar_historial` («Revisar historial»): significa problema con las cuentas de los últimos años o con los datos, no que la empresa sea de mala calidad. El glosario pasa a ser la sección 9.
